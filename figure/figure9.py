...
_, _, _, _, X_test, y_test = load_dataset(data_path='dataset/TIMIT/timit_mel_spikegram_240.pkl')

X = np.concatenate(X_test, axis=0)[:, :72]
mel = X[:, :40].T
spike = X[:, 40:].T
...
h, _, _, im = plt.hist2d(mel_x, mel_y, bins=(40, 1000), range=[[0, 40], [-3, 3]], cmap='binary', vmax=300)
fig.colorbar(im)
...
print(spike_y.min(), spike_y.max())
plt.subplot(2, 1, 2)

h, _, _, im = plt.hist2d(spike_x, spike_y, bins=(32, 1000), range=[[0, 32], [-3, 3]], cmap='binary', vmax=300)
fig.colorbar(im)
